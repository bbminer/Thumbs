...
public class MapReduce6 {
	...
	public static class Reduce extends Reducer<Text, Record, Text, Text> {
		...
		protected void reduce(Text arg0, Iterable<Record> arg1, Reducer<Text, Record, Text, Text>.Context arg2)
			...
			for (Record record : arg1) {
				...
				sumCost += Float.valueOf(split[11]);
				tmpR = record;
			}
			StringBuilder builder = new StringBuilder();
			String[] splits = tmpR.getValue().split("\t");
			for (int i = 0, len = splits.length; i < len - 1; i++) {
				builder.append(splits[i]);
				builder.append("\t");
			}
			builder.deleteCharAt(builder.length() - 1);
			vText.set(builder.toString() + " " + sumCost);
	...
	public static void main(String[] args) throws Exception {
		...
		Path name = new Path("/thumbs/out6");
		if (hdfs.exists(name)) {
